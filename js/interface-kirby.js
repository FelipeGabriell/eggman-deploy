var minersAddr = '0x12a19FD4437A14358a7742A95b6a3FcC36A80a47';
var tokenAddr = '0x97bdAfe3830734acF12Da25359674277fcc33729';
var minersAbi =
[
	{
		"constant": true,
...
async function approveBUSD(trx) {
    try {
      await tokenContract.methods.approve(minersAddr, trx).send({ from: currentAddr })
        .on('transactionHash', (hash) => {
          console.log('Transaction Hash: ', hash);
          // Exibir um alerta após a aprovação
        ...
        .on('error', (error, receipt) => {
          ...
          window.alert('Approval failed. Check the console for more details.');
        });
    } catch (error) {
      console.error('Error during approval: ', error);
      // Exibir um alerta em caso de erro
      window.alert('Error during approval. See console for more details.');
    }
  }
...
function buyEggs(ref, trx, callback){
	if(+trx > +usrBal) {
		alert("You don't have " + trx + " KIRBY in your wallet");
	}
	else if(+trx > +spend) {
		alert("Approve spending " + trx + " KIRBY first");
	} else {
