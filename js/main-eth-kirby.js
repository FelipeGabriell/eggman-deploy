...
window.addEventListener('load', async function () {
    ...
    ref1.textContent = window.location.origin + "/kirby/index.html?ref=" + "XX" + encr.toString();
})
...
function refreshData() {
    ...
    contractBalance(function (result) {
        ...
        balanceElem.textContent = stripDecimals(rawStr, 3) + ' KIRBY';
    });

    var userBalanceElem = document.getElementById('user-balance');
    userBalance(function (result) {
        rawStr = numberWithCommas(Number(result).toFixed(3));
        userBalanceElem.textContent = stripDecimals(rawStr, 3) + ' KIRBY';
    });
